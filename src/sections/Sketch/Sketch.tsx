...
type SketchStage = 'waiting' | 'lead' | 'ink' | 'text' | 'complete'

function Sketch() {
  const { t } = useLanguage()
  const { ref, isInView } = useInView<HTMLElement>(0.2)
  const [stage, setStage] = useState<SketchStage>('waiting')

  useEffect(() => {
    if (isInView) {
      setStage((current) => (current === 'waiting' ? 'lead' : current))
      return
    }
    ...
    >
      <Container className={styles.container}>
        <header className={styles.heading}>
          ...
          <h2 className={styles.title} id="sketch-title">
            {t.sketch.title}
          </h2>
        </header>
